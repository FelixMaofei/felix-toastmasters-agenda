...
def render_meta(
    ...
    cells = [
        ...
        (
            "book",
            localized("今日一词", "Word of the Day", language),
            meeting.get("word_of_day") or "-",
            " word",
        ),
        (
            "user",
            localized("会议经理", "Meeting Manager", language),
...
def render_timeline(result: dict[str, Any], language: str) -> tuple[str, str]:
    ...
    for section, items in phase_groups(timeline):
        ...
        for item in items:
            ...
            if details:
                label += f'<span class="item-detail">｜{escape(details)}</span>'
            featured = (
                item.get("id") == feature_item
                or item.get("type") == "special"
                or float(item.get("duration") or 0) >= 20
            )
            row_class = "featured" if featured else ""
            rows.append(
...
def support_panel(title: str, icon_name: str, body: str) -> str:
    return render_panel(title, icon_name, f'<div class="support-body">{body}</div>', "support-panel")
...
def render_support_panels(
    ...
    if "toastmasters_intro" in components:
        panels.append(
            support_panel(
                localized("头马国际介绍", "Toastmasters International", language),
                "world",
                f"<p>{escape(toastmasters_intro_text)}</p>"
    ...
    if "meeting_boundaries" in components:
        panels.append(
            support_panel(
                ...
                "<ul>"
                + "".join(
                    f"<li>{escape(line)}</li>" for line in meeting_boundary_lines
                )
                + "</ul>",
            )
        )
...
def render_editorial_html(
    ...
    *,
    ...
    body = f"""
<main class="page visual-{escape(visual_theme)}">
  <header class="hero">
  ...
  <section class="support-grid support-count-{support_count}">{"".join(panels)}</section>
  <div aria-hidden="true"></div>
  <footer class="footer">
    ...
    return template.replace("{{DOCUMENT_TITLE}}", escape(document_title)).replace("{{BODY}}", body)
