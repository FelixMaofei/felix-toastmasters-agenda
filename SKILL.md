---
name: felix-toastmasters-agenda
description: 将 Toastmasters 角色接龙、会议说明或旧会单参考，转成内容准确、时间闭合、符合官方品牌规范的中英双语会单。适用于各俱乐部的例会议程、角色接龙转会单、Toastmasters agenda、时间重算、A4 打印版和线上分享版；不复刻各俱乐部旧样式。
---

# Felix Toastmasters Agenda V2

## 核心价值

> **继承稳定内容，识别本期变化，自动完成加减计算，让整场会议重新闭合。**

这里的“稳定内容”包括 Toastmasters 共通规则、俱乐部名称/默认地点/语言，以及用户选择长期保留的固定信息组件；不是要求用户每期重新上传上一期会单，也不是照抄某家俱乐部的旧版式。

## 任务边界

- 内容准确和时间闭合优先于视觉创意。
- 统一使用 Toastmasters 品牌色、官方标识和清晰的 A4 信息设计，不还原不同俱乐部的历史样式。
- 最终文字、HTML、PDF 和分享图必须来自同一份计算后 JSON；不要让图片模型重新抄写密集文字。
- 用户只要提供本期接龙和变化。旧会单是可选参考，不是初始化前提；已经提供时，可用于识别俱乐部身份、当届官员和常用固定信息组件的候选值，确认后才保存。
- 姓名已经填进本期接龙，就代表本期承诺；不要再次审问是否本人同意。

## 轻量初始化

用户的最短入口可以只是：

> 帮我把这份角色接龙做成会单。

不要先要求用户准备旧会单、配置文件或 JSON。首次使用某俱乐部时：

1. 先检查当前消息、已粘贴的接龙、当前附件及其文件名，主动识别俱乐部名称、地点、语言、会议日期时间和当届官员；已有事实不再问。
2. 当前附件里有旧会单就利用；没有就继续，不要让用户中断任务去找。
3. 只把无法从当前材料判定、且真正影响成品的信息合并成一次短询问。不暴露字段名，不让用户填技术表单。
4. 固定信息区用人话询问：“常用四项（时间官规则、头马介绍、四类禁忌、官员团队）、纯议程，还是自选？”推荐只是选项，不替用户作答。

如果确实需要问，开头先告诉用户自己已经识别出什么，再列出尚缺项。可顺带说“有旧会单可以发，没有也不影响”，但不得将它列为必交材料。

## 默认工作流

1. 先从当前消息和附件主动识别俱乐部与会议信息，再读取本期角色接龙和特殊环节，保留接龙中的先后顺序。
2. 区分：
   - 标准环节；
   - 角色触发环节；
   - 本期明确增加的特殊环节；
   - 只显示在幕后团队、不进入时间轴的幕后角色。
3. 一次性询问真正影响成品的缺失信息。通常只包括：
   - 会议日期、起止时间或地点确实未知；
   - 俱乐部名称或语言无法从当前材料识别；
   - 标准环节的负责人未知；
   - 接龙中已有角色但姓名为空、🌺、待定或招募中；
   - 特殊环节缺少时长或位置；
   - 同一角色在宣言和报告中出现不同姓名；
   - 本次要加入哪些固定信息组件尚未明确。
4. 普通例会只需读取 [输入结构](references/input-schema.md)，生成只包含本期事实的 meeting JSON。不要让模型重写完整时间轴、开始时间和转场。只有本期规则冲突、特殊会议或生成器错误无法理解时，才读取 [会单业务与时间规则](references/agenda-rules.md)。
5. Skill 被加载时会显示 Base directory。将它作为 `SKILL_DIR`，用绝对路径运行确定性生成器，不要假设当前目录就是 Skill，也不要手算最终时间：

   ```bash
   python3 "$SKILL_DIR/scripts/build_agenda.py" meeting.json --output-dir output
   ```

6. 生成器会输出：
   - `agenda.computed.json`：唯一内容真源；
   - `agenda.md`：用户确认文字；
   - `agenda.html`：符合 Toastmasters 品牌规范的单文件 A4 会单。
7. 如果生成器报告超时、未闭合、缺负责人或角色冲突，先解决问题，不得绕过检查制作最终成品。
8. 先展示 `agenda.md`，取得文字确认后再导出 PDF/PNG。用户明确选定后停止继续改版。

文字确认后，使用同一 HTML 导出打印和分享文件：

```bash
python3 "$SKILL_DIR/scripts/export_a4.py" output/agenda.html --output-dir output
```

## 执行纪律

- `scripts/build_agenda.py` 和 `scripts/export_a4.py` 是确定性程序，普通任务直接运行。
- **不要阅读、复述或分析 Python 源码，不要读取测试文件。**只有命令出现 Python traceback、无法执行或输出结构损坏时，才检查对应脚本。
- 不要在运行程序之前或之后手算整场时间，也不要用模型自己的计算覆盖 `agenda.computed.json`。
- 程序退出码为 `0` 时，直接读取 `agenda.md` 和诊断摘要；不要为了“再确认一次”展开全部源码。
- 程序退出码为 `2` 时，只根据错误里的缺失项、超时或未闭合分钟数处理；不要猜测修复。
- `export_a4.py` 因内容生成第 2 页而退出 `2` 时，向用户说明单页容量冲突，请其精简议程行、介绍文字或固定信息组件后重新生成。不得绕过导出器直接打印 HTML。
- 必须先检查当前消息和当前附件，不得看都不看就追问。当前输入足够时走最短路径；不因初始化扩张搜索无关历史任务、会员库或网络。

## 标准环节

以下环节默认存在，只有本期明确取消时才在 `standard_overrides` 中设置 `enabled: false`：

- 规则介绍；
- 会长致辞；
- 总主持开场；
- 嘉宾介绍；
- 合影／中场休息／茶歇；
- 真情分享；
- 颁奖；
- 闭幕致辞。

会长致辞和闭幕默认同一位会长。嘉宾介绍和真情分享没有负责人时集中询问。合影/休息的负责人优先使用本期拍照官；拍照官仍属于幕后角色，不会自行创造台前环节。

## 固定信息组件

Skill 内置以下组件，但不强制全部加入。“内置”只表示程序已有标准内容，不表示会自动出现在成品中。首次使用、没有已确认的俱乐部常用选择，或本期想改变时，集中询问用户：

- `timer_rules`：时间官绿黄红牌与响铃规则；
- `toastmasters_intro`：头马国际演讲会介绍；
- `meeting_boundaries`：会议秩序与政治、宗教、色情、传销四类内容禁忌；
- `officers`：当届官员团队；
- `club_intro`：俱乐部介绍；
- `join_info`：如何入会；
- `vpm_qr`：VPM 入会咨询二维码；
- `voting_qr`：本期投票二维码。

首次询问时，可以把前四项作为推荐选项展示。推荐不等于自动加入；未经用户选择，不得写入。用户可以全选、部分选择或传入空数组只要纯议程。

按以下顺序取值，不猜测、不合并：

1. `meeting.support_components` 存在：完整采用它，包括 `[]`；
2. 否则 `club.support_components` 存在：完整采用它，包括 `[]`；
3. 两处都不存在：询问用户；
4. 单期数组是完整覆盖，不与俱乐部数组合并。

用户说“以后通常都要”时，写入 `club.support_components`；说“只改这一期”时，写入 `meeting.support_components`，不改俱乐部长期选择。`[]` 只不显示固定信息区，不会删除会长致辞、嘉宾介绍等标准议程环节。

- 选择 `officers` 时，必须提供当前七类核心官员。
- 选择 `club_intro` 或 `join_info` 时，必须提供对应文字。
- 选择二维码组件时，必须使用用户提供的原始 PNG/JPG/SVG/WebP 文件；不得从旧 PDF/截图裁切、让模型重画或修复二维码。
- 用户需要 Pathways、嘉宾参与规则、俱乐部特色或其他未内置信息时，使用 `custom_support_blocks`，不为每个新内容改造模板。自定义块使用标题和纯文本行，可选 `auto`、`left` 或 `bottom` 位置；默认 `auto` 由排版器装箱。
- 排版器按组件体量优先填充左栏，再流向底部；不把“推荐四项”写死成唯一版式。新组件导致单页超载时，由导出器阻断并请用户取舍。

## 接龙触发规则

- 角色行存在且姓名完整：生成对应环节。
- 角色行存在但姓名未定：环节保留，负责人待补。
- 整条角色没有出现：不生成该角色对应的宣言或报告。
- 时间官、哼哈官、语法官分别触发“宣言/介绍 + 报告”，两处姓名必须一致。
- 备稿演讲和备稿点评按编号对应。默认需要点评；接龙明确没有点评时，在本期事实中设置 `evaluation_enabled: false`。不要用“漏写 evaluator 字段”代表取消。
- 即兴点评时长与即兴演讲联动，通常约为即兴演讲时长的一半，不能固定写死。
- 场控、打印、签到、直播、投票链接等幕后角色只进入幕后团队。

## 时间与超时

- 默认正式例会为 120 分钟；用户明确给出其他时间窗时，以本期时间窗为准。
- 转场必须参与计算。大多数主要环节后默认 1 分钟；规则介绍、会长致辞以及中场休息之后默认不加转场。
- 时长与转场支持 `0.5` 分钟递增。用户给出 0.5 分钟时直接记录，不得通过删除另一个真实转场来绕过。两个 0.5 分钟自然累计为 1 分钟；半分钟节点用 `HH:MM:30` 显示。
- 备稿点评、即兴点评、官员报告等角色触发环节也可通过 `transition_overrides` 按实际环节 ID 单独覆盖；不得因程序默认值修改会议事实。
- 即兴演讲、即兴点评、中场休息和真情分享是联动弹性区，不设置即兴演讲的硬范围。
- 未锁定的即兴点评不能被当成任意填充时间；自动计算仍须保持在即兴演讲一半附近。若因其他时长锁定而无法闭合，报告剩余或超出分钟数，不得把即兴点评拉长补洞。
- 程序自动调整休息和真情分享时只在实用范围内变化；用户明确锁定的本期时长可以超出该自动范围。
- 计算结果超出结束时间时，必须告诉用户超出多少分钟，并请用户决定延长或删减；不得偷偷延长、压缩或修改已确认内容。
- 任意插入、删除、换序或改时长后，重新运行生成器，重算后续全部时间点。

## 视觉与交付

- 使用确定性 HTML，而不是图片模型排密集文字。
- 必须使用 Toastmasters International 官方 Logo、Wordmark 或 Logo Lockup；不重新设计 Logo，不自创俱乐部 Logo 或官方 tagline。
- Logo 必须完整、清晰并保留呼吸空间；不拉伸、不改色、不加特效，不遮挡人物脸部或主标题。
- 整体气质保持专业、清晰、温暖、自信和国际化。品牌规范是底线，不是封闭模板；版式、构图、主题表达与视觉节奏可以大胆创新。
- 颜色以官方品牌色为基础并允许适度扩展；可使用深蓝、酒红、浅灰、白色和清晨暖黄，但不偏离品牌气质。避免霓虹色、廉价渐变、卡通配色和过度年轻化的模板感。
- 中文使用清晰专业的黑体类字体，英文使用现代无衬线字体。不使用卡通体、手写体、复古花体或过度科技感字体。
- 最终 PDF 必须且只能是 1 页 A4 竖版。固定信息区与议程整合在同一页；内容放不下时阻断导出并请用户精简，不自动跨页。
- 不得通过裁切、隐藏后半段议程或缩成无法现场阅读的小字来伪造单页成功。
- 中文、英文和中英双语共用同一信息结构，只切换标签语言。
- HTML 是打印与线上传播的共同来源，只能导出 1 页 PDF 和 1 张 PNG。
- 最终文件不得包含提示词、内部判断、缺失项说明、历史会单文字或制作过程备注。

## 不要做

- 不要求用户每期重新提供旧会单。
- 不要求首次用户必须提供旧会单或自己填配置。
- 不为每个俱乐部保存多套流程模板。
- 不复刻旧会单视觉风格。
- 不把接龙里没有的角色型环节擅自补回。
- 不把拍照官、场控等幕后角色直接当成台前环节。
- 不让模型手算后跳过程序验证。
- 不在超时状态下输出“最终版”。
