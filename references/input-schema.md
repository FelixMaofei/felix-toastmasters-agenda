...
- `language`：`zh` 或 `en`；首次初始化或未加载 profile 时必填。V3 不生成中英混排会单。
- 当前材料明显是中文或英文时，AI 直接填写语言；只有无法判断时才询问。

生成器会先合并 profile，再做必填验证。因此使用 `--club-profile` 后，日常 meeting JSON 可以完全省略 `club`，只保留本期事实。

俱乐部首次初始化至少保存这三个身份字段。最小 JSON 故意省略 `support_components`，提醒 AI 在运行前根据 profile、用户要求或本期初版推荐组合写入明确数组；`[]` 表示只要纯议程。
...
固定信息组件必须在内部 JSON 中明确，可以为空，但不一定要把它变成用户的必要问题。字段缺失表示 AI 仍需根据下方顺序形成一个本期数组；`[]` 表示只要纯议程，不显示固定信息区。

取值顺序：

1. `meeting.support_components` 存在：完整采用它，包括 `[]`；
2. 否则 `club.support_components` 存在：完整采用它，包括 `[]`；
3. 两处都不存在：本期先使用 `timer_rules / toastmasters_intro / meeting_boundaries`，已有官员名单时再加入 `officers`；
...
上述推荐组合只影响本期初版，不自动写入长期 profile。用户看图后可以全选、部分选择、加入其他组件或使用空数组只生成议程页。
...
普通首次制作由 `first` 自动生成 `agenda.view.json`。它只表达本期强调的环节、显示列、运营/背景组件顺序、密度、字号档位和对比度；不进入 meeting JSON，也不复制姓名、时间和正文。
...
```bash
python3 scripts/run_agenda.py first meeting.json --output-dir output
```

- 退出码 `0`：负责人完整、时间闭合，并生成内部计算结果与完整 A4 预览包。
- 退出码 `2`：输入、负责人、角色关系或时间闭合失败；按错误修正或向用户确认。

用户看图后可以修改，或运行 `final` 立即交付同一份 PDF/PNG。`draft / preview` 仅用于维护、排错或纯视觉修改。只有实际 PDF 为 1 页 A4 竖版时才能交付；如果浏览器排版产生第 2 页，导出器会删除无效 PDF 并退出 `2`。
