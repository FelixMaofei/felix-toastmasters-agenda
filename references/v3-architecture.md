...
用户有两个真实决策点，但不是两道形式审批：

- 设计前，只回答会改变会议承诺的必要问题；
- 看图后，决定修改还是直接交付。

`agenda.md` 仍是内容层的检查真源，但默认由 AI 自查，不要求用户阅读长表并回复“确认”。AI 不能用自动检查代替人的审美判断；人类通过真实图片保留最终判断权。
...
用户不必默认阅读整份 `agenda.md`。如果必要信息缺失，AI 先说明已识别内容和当前进度，把必要问题一次问完，并把主题、Pathways、介绍、二维码等可选增强单独列出。必要信息齐全且程序检查通过后，直接进入视觉制作。
...
HTML 是内部文件。默认给用户看由最终 A4 页面真实渲染出的图片初版：
...
图片出现后只提供一个合并入口：用户可以一句话修改，也可以回复“直接导出”。视觉反馈只留在本层，不重新计算会议事实；内容反馈回到第一层重新计算。

同一俱乐部明确要求“沿用已经确认的样式直接出”时，可以不再询问审美意见；程序仍须生成并检查预览，不能跳过视觉层。
...
- 第二阶段真实预览时同步生成并校验一页 A4 PDF 与高清 PNG；
- 用户选择直接交付后，校验预览 HTML/PDF/PNG 的哈希并原样提升为正式文件，不再次启动浏览器渲染；
- 嵌入或打包字体和官方品牌资产；
- 保证 PDF 与 PNG 内容一致；
- 使用稳定、清楚的文件名；
- 失败时保留上一版，不交付半成品。

本层不得重新选择重点、调整内容、改变设计或重新渲染。
...
AI 自查内容；必要冲突才回问用户
                ↓
`first` 生成受约束的默认 agenda.view.json
                ↓
单一 semantic HTML renderer + 一套已确认设计系统
                ↓
agenda.preview.html + agenda.preview.pdf + agenda.preview.png
                ↓
用户修改或直接交付
                ↓
校验 manifest 与文件哈希后原样提升
                ↓
...
| `agenda.view.json` | `first` 生成默认视觉意图；用户反馈后 AI 可受控调整，不对用户暴露 |
| `agenda_renderer.py` | 根据视觉意图，用单一语义结构确定性生成确认稿 |
| `assets/agenda.css` | 一套经确认的视觉系统和打印字体 |
| `export_a4.py` | 文件导出与格式完整性检查 |
| `run_agenda.py` | `first` 一次生成初版；`final` 原样交付；保留分段命令供维护 |
...
- AI 在关键事实未清楚时直接生成，或把整篇文字稿变成多余审批门；
- 内容变化和视觉变化互相污染；
- 版式路由选错导致整页方向错误；
- 每种会议维护一套模板导致复杂度持续增长；
- 技术检查通过却视觉失败；
...
1. **流程先行**：改写 `SKILL.md`，只保留必要信息确认与看图后的修改/交付选择；
2. **确定 V3 视觉基线**：同一 renderer 同时通过普通例会、60 分钟重点环节、角色密集 / 英文会单三份真实压力样本；它们是测试样本，不是三套模板；
...
技术切片逐步验证；这不等于要求普通用户逐步审批。

## 9. 停止条件

- 必要信息已确认且时间闭合后，内容层停止追问并直接进入视觉初版；
- 用户回复“直接导出 / OK / 就这样”后，视觉层停止修改；
