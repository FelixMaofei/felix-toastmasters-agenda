...
普通会单直接按 `SKILL.md` 调用 `first`。只有当前材料包含特殊环节、顺序变化、非默认时长、辅助信息或视觉修改，而你不确定字段怎样表达时，才读本文件。
...
`meeting.end` 可省略，程序默认会议总长 120 分钟。`approved_overtime_minutes` 只有用户明确同意程序算出的准确超时分钟数后才可填写。
...
`impromptu` 省略或为 `null` 表示本期没有即兴。`evaluator` 省略表示本期没有即兴点评；`evaluator: null` 表示需要点评但人选未知。未锁定 `minutes` 与 `evaluation_minutes` 时由程序按剩余时间自动闭合。
...
辅助信息不阻断初版。未提供时由程序先使用 profile；仍未提供时自动采用时间官规则、头马介绍和会议边界，如果 profile 中已有完整七人官员团队，再自动加入官员信息。Agent 不要自行拼装这组默认值。用户明确只要纯议程时写空数组：
...
显式 patch 成功后会保存到输出目录，后续 `first` 会自动沿用。用户要求恢复默认时，移除保存的 patch 后再运行 `first`。

## 长期资料更新

只有用户明确说“以后默认都这样”时，才在 `first` 中增加 `--update-club-profile`。本期角色、日期、会议号、特殊环节、临时时长和本期顺序永远不写入 profile。
